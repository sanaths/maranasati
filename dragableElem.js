...
chrome.storage.local.get(
  ["sleepTime", "currentPosition", "isCollupsed", "drag_elem_showing"],
  async ({ sleepTime, currentPosition, isCollupsed, drag_elem_showing }) => {
    console.log(sleepTime, currentPosition, isCollupsed, drag_elem_showing);

    //check if the current status is hidden or not

    if (drag_elem_showing !== undefined && drag_elem_showing === "showing") {
      const checkDragElem = document.getElementById(
        "ext__dragable_main_container"
      );

      if (checkDragElem === null) {
        ...
        dragable_main_container.setAttribute(
          "id",
          "ext__dragable_main_container"
        );
        ...
        if (currentPosition !== undefined) {
          ...
          dragable_main_container.style.left = "100px";
          dragable_main_container.style.top = "100px";
        }
        ...
        iconImage.setAttribute(
          "src",
          "https://maranasati.s3.amazonaws.com/yama.gif"
        );
        ...
        let dragImg = false;
        iconImage.addEventListener("mousedown", () => (dragImg = false));

        iconImage.addEventListener("mousemove", () => (dragImg = true));
        ...
        iconImage.addEventListener("mouseup", () => {
          if (dragImg) {
            ...
            if (timerArea.classList.contains("d_none")) {
              ...
              dragable_main_container.style.backgroundColor = "rgb(81,7,37)";
        ...
        const dragElem = document.getElementById(
          "ext__dragable_main_container"
        );
        ...
        function dragElement(elmnt) {
          ...
          async function closeDragElement() {
            ...
            await chrome.storage.local.set({
              currentPosition: {
                bottom: currentPositionFromDrag.bottom,
                left: currentPositionFromDrag.left,
                right: currentPositionFromDrag.right,
                top: currentPositionFromDrag.top,
              },
            });
          }
        }
...
chrome.storage.local.get("sleepTime", ({ sleepTime }) => {
  setInterval(function () {
    ...
    const dragable_elm_secoundLeftDom = document.getElementById(
      "ext_timer_area_seconds"
    );
